...
def main(is_semi_supervised, trial_num):
    ...
    n = x.shape[0]
    group_idxs = np.random.randint(0, K, size=n)
    groups = [x[group_idxs == k] for k in range(K)]
    mu = [group.mean(axis=0) for group in groups]
    sigma = [np.cov(group, rowvar=False) for group in groups]
    phi = np.ones(K) / K
    w = np.ones((n, K)) / K
    # *** END CODE HERE ***
...
def run_em(x, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        def get_p_x_bar_z():
            ...
            return p_x_bar_z
        p_x_bar_z = get_p_x_bar_z()
        p_x_and_z = p_x_bar_z * phi
        w = p_x_and_z / p_x_and_z.sum(axis=1, keepdims=True)
        sigma = np.array([sum(w[i][k] * np.outer(x_norms[k][i], x_norms[k][i]) for i in range(N)) / w[:,k].sum() for k in range(K)])
        phi = w.sum(axis=0) / N
        mu = [w[:, k].dot(x) / w[:, k].sum() for k in range(K)]
        prev_ll = ll
        ll = np.log(get_p_x_bar_z().dot(phi)).sum()
