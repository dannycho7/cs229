...
2. Every problem has its own directory (*e.g.,* `tex/stability/` for Problem 1).
3. Every subproblem has two files within the parent problem’s directory:
  - The problem statement, *e.g.* `tex/stability/01-a-vs-b.tex` for Problem 1(a)). You do not need to modify this.
  - Your solution, *e.g.* `tex/stability/01-a-vs-b-sol.tex` for your solution to Problem 1(a). You will need to modify these files (and the source files in `src` for coding parts).
